export default function Train3() {
	return (
		<svg
			version="1.1"
			id="Layer_1"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 512 512"
			xmlSpace="preserve"
		>
			<circle style={{ fill: '#88C5CC' }} cx="256" cy="256" r="256" />
			<path
				style={{ fill: '#80B9BF' }}
				d="M474.752,389.04L355.484,91.672c-1.428-5.188-5.98-9.004-11.392-9.708l-58.508-4.48
	c-0.384-0.92-0.86-1.64-1.412-2.408c-0.204-0.724-0.448-1.336-0.748-1.968c-0.164-0.556-0.344-1.064-0.564-1.556
	c-0.22-0.896-0.524-1.688-0.876-2.452c-0.4-1.36-0.964-2.532-1.684-3.56c-0.084-0.2-0.176-0.36-0.26-0.552
	c-0.164-0.572-0.356-1.112-0.584-1.62c-2.3-9.284-11.932-11.392-23.472-11.392c-0.676,0-1.34,0.016-2,0.016c0,0,0.004,0,0.008,0
	c-12.32,0-22.008,3.412-22.008,15.984c0,2.948,0.56,5.752,1.532,8.364c0.064,0.244,0.124,0.484,0.2,0.728l-65.832,5.376
	C161.356,83.3,156,89.4,156,96v2.048v2.056v53.708l-8.084,0.536C139.16,155.252,132,163.2,132,172v2.048v2.056V224v2.048v2.056V252
	v2.048v2.056V340h-8v4v63.952V410v2l31.864,79.66C186.624,504.744,220.464,512,256,512C348.66,512,429.812,462.764,474.752,389.04z"
			/>
			<path
				style={{ fill: '#CC584C' }}
				d="M356,160V96c0-6.6-5.356-12.696-11.9-13.548l-76.104-6.148C261.4,76.136,250.6,76.108,244,76.244
	l-76.1,6.212C161.356,83.304,156,89.4,156,96v64H356z"
			/>
			<path
				style={{ fill: '#BF5347' }}
				d="M344.1,82.452l-76.104-6.148c-5.308-0.136-13.3-0.176-19.672-0.116
	c2.812,0.024,5.48,0.064,7.672,0.116l76.104,6.148C338.644,83.304,344,89.4,344,96v64h12V96C356,89.4,350.644,83.304,344.1,82.452z"
			/>
			<path
				style={{ fill: '#C2E2F2' }}
				d="M172,156V98.172l72.812-5.936c2.668-0.052,5.968-0.076,9.36-0.076c4.6,0,9.376,0.044,12.984,0.128
	L340,98.176V156H172z"
			/>
			<path
				style={{ fill: '#CCEEFF' }}
				d="M180,160v-54.452l65.212-5.32c2.58-0.048,5.728-0.068,8.96-0.068c4.436,0,9.04,0.04,12.572,0.12
	L332,105.556V160H180z"
			/>
			<rect
				id="SVGCleanerId_0"
				x="248"
				y="100"
				style={{ fill: '#C2E2F2' }}
				width="16"
				height="52"
			/>
			<path
				style={{ fill: '#CC584C' }}
				d="M380,340V172c0-8.8-7.16-16.752-15.916-17.66l-92.088-6.012c-8.796-0.18-23.196-0.212-31.996-0.064
	l-92.088,6.076C139.16,155.252,132,163.2,132,172v168H380z"
			/>
			<path
				style={{ fill: '#BF5347' }}
				d="M364.084,154.34l-92.088-6.012c-7.068-0.144-17.704-0.188-26.192-0.124
	c3.732,0.024,7.28,0.072,10.192,0.124l92.088,6.012C356.84,155.252,364,163.2,364,172v168h16V172
	C380,163.2,372.84,155.252,364.084,154.34z"
			/>
			<path
				style={{ fill: '#994239' }}
				d="M268,224v-75.744c-8.728-0.112-24-0.256-24-0.256v76H132v28h108h28h112v-28H268z"
			/>
			<circle style={{ fill: '#F5F5F5' }} cx="256" cy="236" r="44" />
			<polygon
				style={{ fill: '#B3B3B3' }}
				points="388,407.952 256,436 124,407.952 124,340 388,340 "
			/>
			<polygon
				style={{ fill: '#A6A6A6' }}
				points="376,340 376,407.952 250,434.728 256,436 388,407.952 388,340 "
			/>
			<g>
				<polygon
					style={{ fill: '#999999' }}
					points="280,430.904 280,340 272,340 272,432.6 	"
				/>
				<polygon
					style={{ fill: '#999999' }}
					points="312,424.1 312,340 304,340 304,425.8 	"
				/>
				<polygon
					style={{ fill: '#999999' }}
					points="348,416.452 348,340 340,340 340,418.152 	"
				/>
				<polygon
					style={{ fill: '#999999' }}
					points="172,418.152 172,340 164,340 164,416.452 	"
				/>
				<polygon
					style={{ fill: '#999999' }}
					points="203.2,424.78 208,425.8 208,340 200,340 200,424.1 	"
				/>
				<polygon
					style={{ fill: '#999999' }}
					points="240,432.6 240,340 232,340 232,430.904 	"
				/>
			</g>
			<circle
				style={{
					fill: 'none',
					stroke: '242424',
					strokeWidth: '4',
					strokeMiterlimit: '10'
				}}
				cx="256"
				cy="236"
				r="44"
			/>
			<circle style={{ fill: '#B3B3B3' }} cx="172" cy="308" r="24" />
			<circle style={{ fill: '#E6E6E6' }} cx="172" cy="308" r="16" />
			<circle style={{ fill: '#B3B3B3' }} cx="340" cy="308" r="24" />
			<circle style={{ fill: '#E6E6E6' }} cx="340" cy="308" r="16" />
			<path
				style={{ fill: '#CC584C' }}
				d="M280,68c0,13.252-10.752,24-24,24c-13.252,0-24-10.752-24-24s10.748-16,24-16
	C269.252,52,280,54.752,280,68z"
			/>
			<path
				style={{ fill: '#BF5347' }}
				d="M256,52c-0.676,0-1.34,0.012-2,0.028c12.316,0.284,22,3.392,22,15.972s-9.684,22.884-22,23.9
	c0.66,0.056,1.324,0.1,2,0.1c13.252,0,24-10.752,24-24S269.252,52,256,52z"
			/>
			<path
				id="SVGCleanerId_1"
				style={{ fill: '#F5F5F5' }}
				d="M276,68c0,11.048-8.952,20-20,20c-11.044,0-20-8.952-20-20s8.956-12,20-12
	C267.048,56,276,56.952,276,68z"
			/>
			<path
				style={{ fill: '#263740' }}
				d="M356,160V96c0-6.6-5.356-12.696-11.9-13.548l-76.104-6.148C261.4,76.136,250.6,76.108,244,76.244
	l-76.1,6.212C161.356,83.304,156,89.4,156,96v64H356z"
			/>
			<path
				style={{ fill: '#1E2C33' }}
				d="M344.1,82.452l-76.104-6.148c-5.308-0.136-13.3-0.176-19.672-0.116
	c2.812,0.024,5.48,0.064,7.672,0.116l76.104,6.148C338.644,83.304,344,89.4,344,96v64h12V96C356,89.4,350.644,83.304,344.1,82.452z"
			/>
			<path
				style={{ fill: '#C2E2F2' }}
				d="M172,156V98.172l72.812-5.936c2.668-0.052,5.968-0.076,9.36-0.076c4.6,0,9.376,0.044,12.984,0.128
	L340,98.176V156H172z"
			/>
			<path
				style={{ fill: '#E6F7FF' }}
				d="M180,160v-54.452l65.212-5.32c2.58-0.048,5.728-0.068,8.96-0.068c4.436,0,9.04,0.04,12.572,0.12
	L332,105.556V160H180z"
			/>
			<g>
				<rect
					id="SVGCleanerId_0_1_"
					x="248"
					y="100"
					style={{ fill: '#C2E2F2' }}
					width="16"
					height="52"
				/>
			</g>
			<path
				style={{ fill: '#263740' }}
				d="M380,340V172c0-8.8-7.16-16.752-15.916-17.66l-92.088-6.012c-8.796-0.18-23.196-0.212-31.996-0.064
	l-92.088,6.076C139.16,155.252,132,163.2,132,172v168H380z"
			/>
			<path
				style={{ fill: '#1E2C33' }}
				d="M364.084,154.34l-92.088-6.012c-7.068-0.144-17.704-0.188-26.192-0.124
	c3.732,0.024,7.28,0.072,10.192,0.124l92.088,6.012C356.84,155.252,364,163.2,364,172v168h16V172
	C380,163.2,372.84,155.252,364.084,154.34z"
			/>
			<g>
				<path
					style={{ fill: '#F5F5F5' }}
					d="M268,224v-75.744c-8.728-0.112-24-0.256-24-0.256v76H132v28h108h28h112v-28H268z"
				/>
				<circle style={{ fill: '#F5F5F5' }} cx="256" cy="236" r="44" />
			</g>
			<polygon
				style={{ fill: '#E6E6E6' }}
				points="404,407.952 256,436 108,407.952 124,340 388,340 "
			/>
			<g>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="280,430.904 280,340 272,340 272,432.6 	"
				/>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="312,424.1 312,340 304,340 304,425.8 	"
				/>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="348,416.452 348,340 340,340 340,418.152 	"
				/>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="172,418.152 172,340 164,340 164,416.452 	"
				/>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="203.2,424.78 208,425.8 208,340 200,340 200,424.1 	"
				/>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="240,432.6 240,340 232,340 232,430.904 	"
				/>
				<polygon
					style={{ fill: '#CCCCCC' }}
					points="376,340 376,407.952 250,434.728 256,436 404,407.952 388,340 	"
				/>
			</g>
			<path
				style={{ fill: '#E78244' }}
				d="M256,288c-28.672,0-52-23.328-52-52s23.328-52,52-52s52,23.328,52,52S284.672,288,256,288z M256,200
	c-19.852,0-36,16.148-36,36s16.152,36,36,36s36-16.148,36-36S275.848,200,256,200z"
			/>
			<circle style={{ fill: '#B3B3B3' }} cx="172" cy="308" r="24" />
			<circle style={{ fill: '#E6E6E6' }} cx="172" cy="308" r="16" />
			<circle style={{ fill: '#B3B3B3' }} cx="340" cy="308" r="24" />
			<circle style={{ fill: '#E6E6E6' }} cx="340" cy="308" r="16" />
			<path
				style={{ fill: '#263740' }}
				d="M280,68c0,13.252-10.752,24-24,24c-13.252,0-24-10.752-24-24s10.748-16,24-16
	C269.252,52,280,54.752,280,68z"
			/>
			<path
				style={{ fill: '#1E2C33' }}
				d="M256,52c-0.676,0-1.34,0.012-2,0.028c12.316,0.284,22,3.392,22,15.972s-9.684,22.884-22,23.9
	c0.66,0.056,1.324,0.1,2,0.1c13.252,0,24-10.752,24-24S269.252,52,256,52z"
			/>
			<g>
				<path
					id="SVGCleanerId_1_1_"
					style={{ fill: '#F5F5F5' }}
					d="M276,68c0,11.048-8.952,20-20,20c-11.044,0-20-8.952-20-20s8.956-12,20-12
		C267.048,56,276,56.952,276,68z"
				/>
			</g>
		</svg>
	)
}
